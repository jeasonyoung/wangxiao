...
$(function(){
	...
	$("a[name='pagecode']").live('click',function(){
		...
		if($(this).html()==currentPage){ alert("这就是当前页");}
			...
			location="../member/questionList?page="+$(this).html();
	});
	});
	$(function(){
		//初始化选择框
		$.ajax({
			...
			success:function(data)
			{
				for(var i=0;i<data.length;i++)
				{
					...
					{
						var exam = data[i].children[j];
						var $option = $('<option value="'+exam.examId+'">'+exam.examName+'</option>');
						$("#examId").append($option);
					}
				}
			}
		});
	...
	function initGrade()
	{
		var value = $("#examId").val();
		if(value!=0)
		{
			$.ajax({
			async:true,
			type:"post",
			url:"../question/gradeTree",
			data:{"examId":value},
			dataType:"json",
			success:function(data)
			{
				for(var i=0;i<data.length;i++)
				{
					var $option = $('<option value="'+data[i].gradeId+'">'+data[i].name+'</option>');
	...
	function search()
	{
		...
		window.location = "../question/questionList?examId="+examId+"&gradeId="+gradeId;
	}
</script>
</head>
<body>
...
<tr>
  ...
  <td bgcolor="#f9f9f9" align="center" width="500" class="xxzx_ber_a"><strong>标题</strong></td>
  <td bgcolor="#f9f9f9" align="center" width="200" class="xxzx_ber_a"><strong>提问时间</strong></td>
  <td bgcolor="#f9f9f9" align="center" width="150" class="xxzx_ber_a"><strong>提问人</strong></td>
  <td bgcolor="#f9f9f9" align="center" width="150" class="xxzx_ber_a"><strong>操作</strong></td>
</tr>
...
<!-- 			 <td colspan="4" class="xxzx_ber_c font_14">
                <div class="wzgk_onec center">
                    	...
                    	<tr> -->
                   <td height="24"  align="center" width="50" class="xxzx_ber_a" name="status"><img src="../images/answer${q.questionStatus }.gif"/></td>
 				   <td  align="center" width="500" class="xxzx_ber_a">${q.questionTitle }</td>
  					<td  align="center" width="200" class="xxzx_ber_a"><fmt:formatDate value="${q.questionAddTime }" type="both" pattern="yyyy-MM-dd HH:mm" /></td>
  					<td  align="center" width="150" class="xxzx_ber_a">${q.student.stuUsername }</td>
  					<td  align="center" width="150" class="xxzx_ber_a"><a href="../question/loadForAnswer?questionId=${q.questionId }" target="_self">回复</a></td>
  					</tr>
<!--                        </table>
                </div>
        </td> 
