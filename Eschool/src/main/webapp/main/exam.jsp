...
<%@ taglib uri="/struts-tags" prefix="s" %>
 <div class="kctext"><img src="/main/images/kctext.jpg" /></div>
  <div class="kccon">
    <ul><s:iterator value="list" var="exam" status="col"><li><div class="kcb1"><a href="" target="_blank">${exam.examName}</a></div>
        <div class="kcb2">
     	<s:iterator value="# exam.children" var="c" status="status"><s:if test="#status.getIndex()<9"><a href="/main/main?examId=${c.examId}" >${c.examName}</a>　|	</s:if><s:else></s:else></s:iterator>
		<s:if test="#exam.children.size>9">
  		...
  		</s:if><s:else></s:else></div><s:else></s:else></li></s:iterator></ul>
  </div>
