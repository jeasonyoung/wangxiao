...
String key = "lq9230u5rjasdkljfasdq23";		// 登陆后在上面的导航栏里可能找到“B2C”，在二级导航栏里有“MD5密钥设置”
...
String v_md5text = MD5.getMD5ofStr(text);
		out.print(v_md5text);
if (v_md5str.equals(v_md5text))
{
	if ("30".equals(v_pstatus))
	{
		...
		request.setAttribute("trade_type", "WY");
		if(v_oid.indexOf("-")!=-1)	//如果订单号经过了特殊处理
		{
			request.getRequestDispatcher("/order/multiPay").forward(request,response);//转发到多次支付进行处理
		}else
		{
			request.getRequestDispatcher("/order/bankpay").forward(request,response);
		}
		%>
