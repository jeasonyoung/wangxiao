...
<%@ page import="com.changh.eschool.pay.tenpay.ResponseHandler" %>   
<%@ page import="com.changh.eschool.pay.tenpay.client.ClientResponseHandler" %>    
<%@ page import="com.changh.eschool.pay.tenpay.client.TenpayHttpClient" %>
		...
		if(queryRes.isTenpaySign()&& "0".equals(retcode)){ 
			...
			if("1".equals(trade_mode)){       //即时到账 
				if( "0".equals(trade_state)){
					...
					//注意交易单不要重复处理
					//注意判断返回金额
					request.setAttribute("trade_type", "CFT");//交易类型（财付通）
					request.getRequestDispatcher("/order/tenpay").forward(request,response);
					//------------------------------
					//即时到账处理业务完毕
					//------------------------------
