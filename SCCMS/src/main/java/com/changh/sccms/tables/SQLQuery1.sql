...
alter table tbRechargeRecord
   add constraint FK_TBRECHARGE_REFERENCE_TBSTUDEN foreign key (stu_id)
      references tbStudent (stu_id);
alter table tbGrade add c
drop table tbMenu
create table tbMenu
(
   menu_id int identity(1001,1) primary key,
...
CREATE TABLE tbRole(
  ...
  role_url  varchar(1000) null,
)

alter table tbAdministor
   add constraint FK_TBADMIN_REFERENCE_TBROLE foreign key (role_id)
      references tbRole (role_id);
select * from tbRole

delete from  tbAdministor;
alter table tbAdministor DROP column menu_id

create table tbWebInfo
(
   webInfo_id     int identity(1001,1)       primary key,
   ...
   webInfo_serviceTel		varchar(50),
   webInfo_officePhone		varchar(50),
   webInfo_comfax           varchar(50),
   webInfo_copyright		varchar(50),
   webInfo_index			varchar(50),
   webInfo_workday			varchar(50)
);
...
CREATE TABLE `t_s_log` (
  ...
  `operatetype` smallint(6) default NULL COMMENT '操作类型：1登录、2退出、3插入、4删除、5更新、6打印、7上传、8其他',
...
create table tbLog
(
   log_id     int identity(1001,1)       primary key,
   adm_id               int ,
   adm_username         varchar(50)    not null,
   operatetime   		datetime          not null,
   operatetype			int     not null,--操作类型：1登录、2退出、3插入、4删除、5更新、6打印、7上传、8其他--
...
select * from tbLog where adm_username like '%jw%';
select count(*) from tbLog where  adm_username like '%jw%'


create table tbNews(
	new_id int identity(1001,1) primary key,
	new_title varchar(500) not null,
	sub_time varchar(500) default null comment '副标题',
	new_content text not null comment '新闻内容',
	click_num int default 1 comment '点击次数',
	is_top  int default 0 comment '是否置顶',
	new_templet varchar(250) default null comment '新闻模板',
	new_path varchar(250) default null comment '路径',
	exam_id int default null comment '考试类别',
	class_id int default null comment '资讯类别',
	order_id int  default 0,
	add_time datetime not null,
	aditor varchar(100) not null comment '添加者',
...
create table tbNews(
	...
	new_templet varchar(250)  null,
	new_path varchar(250)  null,
	exam_id int null,
	class_id int null,
	order_id int default 0,
	add_time datetime not null,
	aditor varchar(100) not null 
)

alter table tbNews add province_id int default null;
alter table tbNews add new_keywords varchar(250) default null;
create table new_class(
	class_id int identity(1001,1) primary key,
	class_ename varchar(100) default null,
	...
	class_temp varchar(200) default null
)
 
alter table new_class add pids varchar(100) default null
create table province(
	province_id int identity(1001,1) primary key,
