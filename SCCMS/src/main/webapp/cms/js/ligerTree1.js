                    ...
                    if (p.autoCheckboxEven)
                    {
                        ...
                        if ($(obj).hasClass("l-checkbox-unchecked"))
                        {
                            ...
                            $(".l-children .l-checkbox", treeitem)
                                    ...
                                    .addClass("l-checkbox-checked");  //看这里，这句话表示选中节点(checkbox)
                            g.trigger('check', [{ data: treenodedata, target: treeitem[0] }, true]); //这句话表示触发oncheck事件
                        }
                        //状态：选中
                        ...
                        g._setParentCheckboxStatus(treeitem); //这句话表示父节点也选中，递归往上
                        if ($(obj).hasClass("l-checkbox-unchecked"))                        
                            g.trigger('aftercheck', [{ data: treenodedata, target: treeitem[0] }, true]); 
                        else if ($(obj).hasClass("l-checkbox-checked"))
                            g.trigger('aftercheck', [{ data: treenodedata, target: treeitem[0] }, false]); 
                    }
                    else
                    {
                        //状态：未选中
