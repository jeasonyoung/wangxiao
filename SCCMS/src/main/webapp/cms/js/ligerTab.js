...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        overrideTabItem: function (targettabid, options)
        {
            ...
            {
            	//g.reload2(tabid, url); 2013.04.19 修改
            	g.reload3(tabid,url,text);
                return;
            }
        ...
        reload2: function (tabid,url2)
        {
            ...
            var url = $(iframe).attr("src");
            iframeloading.show();
            iframe.attr("src", url2).unbind('load.tab').bind('load.tab', function ()
            {
                iframeloading.hide();
            });
        },
        //2013.04.20 fw 添加一个方法替换原来text
        reload3:function(tabid,url2,text)
        {
        	var g = this, p = this.options;
            var contentitem = $(".l-tab-content-item[tabid=" + tabid + "]");
            var iframeloading = $(".l-tab-loading:first", contentitem);
